...
def simple_raycast(orig, dir, objects: List[Object], depth):
    ...
    if neirest_object is not None: 
        ...
        for light in LIGHTS:
            ...
            specular_intensity += max(0, np.dot(reflect(N, light_dir), dir)) ** \
                               neirest_object.material.specular_exponent * light.intensity 

        color = neirest_object.material.albedo_color * light.intensity * 0.2 + \
        ...
        color += simple_raycast(reflect_orig, reflect_dir, objects, depth=depth+1) * neirest_object.material.reflection
